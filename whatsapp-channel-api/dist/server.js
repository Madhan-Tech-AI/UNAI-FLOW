...
app.get('/v1/whatsapp/:connectionId/channels', async (req, res) => {
    try {
        ...
        const allChannels = await newsletter_service_js_1.NewsletterService.discoverChannels(session.socket, connectionId);
...
app.post('/v1/whatsapp/:connectionId/channels/resolve', async (req, res) => {
    try {
        ...
        const channel = await newsletter_service_js_1.NewsletterService.resolveChannel(session.socket, link_or_code);
        if (!channel) {
