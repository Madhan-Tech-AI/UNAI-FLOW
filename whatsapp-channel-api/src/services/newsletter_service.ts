...
export interface DiscoveredNewsletter {
  id: string; // Invite code or JID (used for UI routing/display)
  channel_id?: string;
  jid?: string; // Real numeric newsletter JID (e.g. 120363171744447809@newsletter)
  newsletter_jid?: string;
  name: string;
  link: string;
...
export class NewsletterService {
   ...
   */
  public static decodeHtmlEntities(text: string | null | undefined): string {
    if (!text) return '';
    return text
      .replace(/&#039;/g, "'")
      .replace(/&apos;/g, "'")
   ...
   */
  public static cleanUrl(url: string | null | undefined): string | null {
    if (!url) return null;
   ...
   */
  public static extractPictureUrl(fullMeta: any, rawItem?: any): string | null {
              ...
              if (jid && !seenIds.has(jid)) {
                ...
                try {
                  if (typeof (sock as any).newsletterMetadata === 'function') {
                    fullMeta = await (sock as any).newsletterMetadata('jid', realJid);
                  }
                } catch (metaErr: any) {
                ...
                const channelName = this.decodeHtmlEntities(
                  fullMeta?.thread_metadata?.name?.text || fullMeta?.name || item.thread_metadata?.name?.text || item.name || 'WhatsApp Channel'
                );
                const description = this.decodeHtmlEntities(
                  fullMeta?.thread_metadata?.description?.text || fullMeta?.description || item.thread_metadata?.description?.text || ''
                );
                const verified = Boolean(
                  (fullMeta?.thread_metadata?.verification === 'VERIFIED') ||
      ...
      try {
        ...
        for (const c of chatStore) {
          ...
          if (jid.endsWith('@newsletter') && !seenIds.has(jid)) {
            ...
            try {
              if (typeof (sock as any).newsletterMetadata === 'function') {
                meta = await (sock as any).newsletterMetadata('jid', realJid);
              }
            } catch (e) {}
        ...
        if (typeof (sock as any).newsletterMetadata === 'function') {
          ...
          if (meta && meta.id) {
            ...
            if (rawSubs !== undefined && rawSubs !== null) {
              ...
              if (!isNaN(parsed)) subscribers_count = parsed;
            }
  ...
  public static async publishToChannel(
    ...
    if (!isNumericNewsletterJid) {
      ...
      if (this.jidCache.has(prefix)) {
        ...
        try {
          ...
          if (typeof (sock as any).newsletterMetadata === 'function') {
            const meta = await (sock as any).newsletterMetadata('invite', prefix);
            if (meta && meta.id) {
              const realJid = meta.id.includes('@') ? meta.id : `${meta.id}@newsletter`;
          ...
          throw new Error(
            `Cannot publish: channel identifier '${channelId}' is not a numeric WhatsApp Channel JID and could not be resolved from WhatsApp. Error: ${resolveErr.message}`
          );
        }
      }
    }
    ...
    if (!/^\d+@newsletter$/.test(normalizedJid)) {
      throw new Error(
        `Cannot publish: invalid WhatsApp Channel JID '${normalizedJid}'. WhatsApp Channels require a valid numeric @newsletter JID.`
      );
    }
